...
CLOUD_INIT_TIMEOUT_SECONDS="${CLOUD_INIT_TIMEOUT_SECONDS:-600}"
...
echo "Waiting for cloud-init to finish before starting installer (timeout: ${CLOUD_INIT_TIMEOUT_SECONDS}s)..."
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$SSH_KEY" -p "$SSH_PORT" unifai@127.0.0.1 "
  if command -v cloud-init >/dev/null 2>&1; then
    sudo timeout ${CLOUD_INIT_TIMEOUT_SECONDS} cloud-init status --wait
  else
