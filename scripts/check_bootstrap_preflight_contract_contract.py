...
required = [
    ...
    ('require_file "$REPO_ROOT/scripts/check_publish_stack_reconciliation_note.py"', 'Bootstrap preflight contract checker requires the publish-stack reconciliation note freshness checker path'),
    ('require_file "$REPO_ROOT/scripts/check_publish_stack_reconciliation_note_contract.py"', 'Bootstrap preflight contract checker requires the publish-stack reconciliation note contract checker path'),
    ...
    ('require_file "$REPO_ROOT/scripts/smoke_test_publish_stack_reconciliation_note.sh"', 'Bootstrap preflight contract checker requires the publish-stack reconciliation note smoke test path'),
    ...
    ('require_file "$REPO_ROOT/scripts/check_vm_verifier_preflight_contract.py"', 'Bootstrap preflight contract checker requires the VM verifier preflight wrapper contract checker path'),
    ...
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_gh_unauthenticated_fallback.sh"', 'Bootstrap preflight contract checker requires the VM verifier unauthenticated-gh fallback smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_tcg_launch.sh"', 'Bootstrap preflight contract checker requires the VM verifier TCG launch smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_kvm_fallback_guidance.sh"', 'Bootstrap preflight contract checker requires the VM verifier kvm-fallback guidance smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_missing_report_fail_closed.sh"', 'Bootstrap preflight contract checker requires the VM verifier missing-report fail-closed smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires the VM verifier failure-excerpt smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_remote_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires the VM verifier remote-failure excerpt smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_remote_failure_missing_report.sh"', 'Bootstrap preflight contract checker requires the VM verifier remote-failure missing-report smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_installer_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires the VM verifier installer-failure excerpt smoke test path'),
    ('require_file "$REPO_ROOT/scripts/smoke_test_vm_verifier_preflight_wrapper.sh"', 'Bootstrap preflight contract checker requires the VM verifier preflight-wrapper smoke test path'),
    ...
    ('python3 -m py_compile "$REPO_ROOT/scripts/check_publish_stack_reconciliation_note.py"', 'Bootstrap preflight contract checker requires py_compile coverage for the publish-stack reconciliation note freshness checker'),
    ('python3 -m py_compile "$REPO_ROOT/scripts/check_publish_stack_reconciliation_note_contract.py"', 'Bootstrap preflight contract checker requires py_compile coverage for the publish-stack reconciliation note contract checker'),
    ...
    ('bash -n "$REPO_ROOT/scripts/smoke_test_publish_stack_reconciliation_note.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the publish-stack reconciliation note smoke test'),
    ...
    ('python3 -m py_compile "$REPO_ROOT/scripts/check_vm_verifier_preflight_contract.py"', 'Bootstrap preflight contract checker requires py_compile coverage for the VM verifier preflight wrapper contract checker'),
    ...
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_gh_unauthenticated_fallback.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier unauthenticated-gh fallback smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_tcg_launch.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier TCG launch smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_kvm_fallback_guidance.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier kvm-fallback guidance smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_missing_report_fail_closed.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier missing-report fail-closed smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier failure-excerpt smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_remote_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier remote-failure excerpt smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_remote_failure_missing_report.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier remote-failure missing-report smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_installer_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier installer-failure excerpt smoke test'),
    ('bash -n "$REPO_ROOT/scripts/smoke_test_vm_verifier_preflight_wrapper.sh"', 'Bootstrap preflight contract checker requires syntax coverage for the VM verifier preflight-wrapper smoke test'),
    ...
    ('python3 "$REPO_ROOT/scripts/check_publish_stack_reconciliation_note.py"', 'Bootstrap preflight contract checker requires runtime coverage for the publish-stack reconciliation note freshness checker'),
    ('python3 "$REPO_ROOT/scripts/check_publish_stack_reconciliation_note_contract.py"', 'Bootstrap preflight contract checker requires runtime coverage for the publish-stack reconciliation note contract checker'),
    ...
    ('python3 "$REPO_ROOT/scripts/check_vm_verifier_preflight_contract.py"', 'Bootstrap preflight contract checker requires runtime coverage for the VM verifier preflight wrapper contract checker'),
    ...
    ('bash "$REPO_ROOT/scripts/smoke_test_publish_stack_reconciliation_note.sh"', 'Bootstrap preflight contract checker requires the publish-stack reconciliation note smoke test'),
    ...
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_gh_unauthenticated_fallback.sh"', 'Bootstrap preflight contract checker requires the verifier unauthenticated-gh fallback smoke test'),
    ...
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_tcg_launch.sh"', 'Bootstrap preflight contract checker requires the verifier TCG launch smoke test'),
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_kvm_fallback_guidance.sh"', 'Bootstrap preflight contract checker requires the verifier kvm-fallback guidance smoke test'),
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_missing_report_fail_closed.sh"', 'Bootstrap preflight contract checker requires the verifier missing-report fail-closed smoke test'),
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires the verifier failure-excerpt smoke test'),
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_remote_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires the verifier remote-failure excerpt smoke test'),
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_remote_failure_missing_report.sh"', 'Bootstrap preflight contract checker requires the verifier remote-failure missing-report smoke test'),
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_installer_failure_excerpts.sh"', 'Bootstrap preflight contract checker requires the verifier installer-failure excerpt smoke test'),
    ('bash "$REPO_ROOT/scripts/smoke_test_vm_verifier_preflight_wrapper.sh"', 'Bootstrap preflight contract checker requires the verifier preflight-wrapper smoke test'),
