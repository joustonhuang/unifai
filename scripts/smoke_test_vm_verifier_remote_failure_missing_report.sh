...
echo "=== UnifAI Smoke Test: VM verifier remote-failure handles missing report ==="

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VERIFY_SCRIPT="$REPO_ROOT/scripts/vm/verify_bootstrap_in_vm.sh"
REAL_BASH="$(command -v bash)"
TMP_DIR="$(mktemp -d -t unifai-vm-remote-missing-report-XXXXXX)"
...
printf 'fake serial with missing report\n' > "$serial_path"
printf 'fake qemu with missing report\n' >&2
...
if [ "$STATUS" -eq 0 ]; then
  echo "[FAIL] Verifier unexpectedly succeeded in remote-failure missing-report path."
  exit 1
fi
...
for needle in \
  ...
  'fake serial with missing report' \
  '[INFO] qemu log tail (' \
  'fake qemu with missing report'
do
  if ! grep -Fq "$needle" <<<"$OUTPUT"; then
    echo "[FAIL] Expected missing-report failure output missing: $needle"
    exit 1
  fi
done

echo "[PASS] Verifier handles missing copied report on remote failure."
