...
required = [
    ...
    ('SELF_MAINTAINED_HANDOFF_PATHS = {', "Refresh helper groups regenerated checkpoint artifacts into one self-maintained handoff set"),
    ('str(CHECKPOINT_LATEST.relative_to(REPO_ROOT))', "Refresh helper treats the latest-checkpoint artifact as self-maintained handoff state"),
    ('str(COMMIT_CANDIDATE.relative_to(REPO_ROOT))', "Refresh helper treats the commit-candidate artifact as self-maintained handoff state"),
    ...
    ('return [path for path in dirty_paths if path not in SELF_MAINTAINED_HANDOFF_PATHS]', "Refresh helper strips regenerated handoff artifacts out of doc-only dirty-state comparisons"),
