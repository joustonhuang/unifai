...
required = [
    ...
    ('INPUT_REF="${1:-main}"', 'Verifier keeps the requested ref separate from the normalized verifier ref'),
    ('REPO_ROOT="${UNIFAI_VM_VERIFY_REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"', 'Verifier allows tests to override the repo root used for remote inspection'),
    ('normalize_github_visible_ref() {', 'Verifier defines a helper to normalize explicit local branch refs'),
    ('github_remote_branch_candidate() {', 'Verifier defines a helper to normalize GitHub-backed remote-tracking refs'),
    ('git -C "$REPO_ROOT" remote get-url "$remote" 2>/dev/null', 'Verifier checks remote-tracking refs against the repo remote configuration'),
    ('printf \'%s\\n\' "$branch"', 'Verifier can normalize GitHub-backed remote-tracking refs down to branch names'),
    ('printf \'%s\\n\' "${ref#refs/heads/}"', 'Verifier strips the refs/heads/ prefix before GitHub SHA resolution'),
    ('REF="$(normalize_github_visible_ref "$INPUT_REF")"', 'Verifier normalizes the requested ref before GitHub API lookups'),
    ...
    ('sha_json="$(github_api "repos/$REPO_SLUG/commits/$REF" 2>&1)"', 'Verifier captures GitHub API failures while resolving the normalized commit ref'),
    ('[FAIL] Could not resolve commit SHA for $REPO_SLUG@$INPUT_REF', 'Verifier fails closed while preserving the requested ref in diagnostics'),
    ("[INFO] If '$INPUT_REF' is a local-only commit, push it first or use a GitHub-visible branch/ref.", 'Verifier explains local-only commit refs are not valid VM verifier inputs'),
    ("SHA=\"$(printf '%s' \"$sha_json\" | jq -r '.sha')\"", 'Verifier resolves commit SHA from captured GitHub API JSON'),
    ('echo "Normalized ref: $REF"', 'Verifier announces when an explicit refs/heads or GitHub remote-tracking ref was normalized'),
