...
required = [
    ...
    ('reject_stale_visible_ref_for_current_branch() {', "Wrapper rejects explicit GitHub-visible refs that are stale relative to the current local branch"),
    ...
    ("resolves to the current GitHub-visible head for '$current_branch', but local branch '$current_branch' is ahead by $ahead commit(s).", "Wrapper fails clearly when an explicit visible ref points at a stale published head"),
    ("Push/reconcile the local tip first, or rerun against the exact published SHA only after the local checkpoint-handoff stack matches that GitHub-visible ref.", "Wrapper explains how to recover from a stale explicit visible ref"),
    ...
    ('python3 scripts/check_github_check_gate.py "$verifier_ref"', "Wrapper runs GitHub check gate inspection on the GitHub-visible verifier ref"),
...
smoke_required = [
    ...
    ('Expected default no-arg case to run the check-gate command on the GitHub-visible verifier ref.', "Wrapper smoke test checks the check-gate step in the default no-arg case"),
    ...
    ('Expected detached-HEAD remote-ref case to run the check-gate command on the GitHub-visible verifier ref.', "Wrapper smoke test explains the detached-HEAD remote-ref check-gate expectation"),
    ('"$WRAPPER" "$VISIBLE_REMOTE_REF"', "Wrapper smoke test exercises the wrapper with an explicit remote-tracking ref"),
    ('python3 scripts/check_github_check_gate.py $VISIBLE_VERIFIER_REF', "Wrapper smoke test runs the check-gate step on the normalized GitHub-visible verifier ref"),
    ...
    ('STALE_FIXTURE_DIR="$(mktemp -d -t unifai-vm-preflight-stale-XXXXXX)"', "Wrapper smoke test creates a separate fixture repo for stale-visible-ref coverage"),
    ('git -C "$STALE_FIXTURE_DIR" remote add origin https://github.com/example/unifai.git', "Wrapper smoke test configures a fake GitHub-backed remote for stale-visible-ref coverage"),
    ('git -C "$STALE_FIXTURE_DIR" update-ref refs/remotes/origin/main "$(git -C "$STALE_FIXTURE_DIR" rev-parse HEAD)"', "Wrapper smoke test seeds a visible remote-tracking ref for stale-visible-ref coverage"),
    ('STALE_VISIBLE_SHA="$(git -C "$STALE_FIXTURE_DIR" rev-parse refs/remotes/origin/main)"', "Wrapper smoke test derives the stale published SHA from the visible remote-tracking ref"),
    ('Expected stale visible SHA case to fail when the local branch is ahead of the published GitHub-visible head.', "Wrapper smoke test explains the stale visible SHA fail-closed expectation"),
    ('Expected stale visible SHA failure message missing.', "Wrapper smoke test checks the stale visible SHA failure message"),
    ('Expected stale visible SHA recovery guidance missing.', "Wrapper smoke test checks the stale visible SHA recovery guidance"),
    ('Stale visible SHA case should fail before any preflight steps are planned.', "Wrapper smoke test guards that stale explicit visible refs fail before planning"),
    ...
    ('Expected explicit refs/heads case to run the check-gate command on the GitHub-visible verifier ref.', "Wrapper smoke test checks the check-gate step in the explicit refs/heads case"),
    ...
    ('Expected -- separator case to run the check-gate command on the GitHub-visible verifier ref.', "Wrapper smoke test checks the check-gate step when ref is passed after --"),
