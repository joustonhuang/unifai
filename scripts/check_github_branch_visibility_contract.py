...
required = [
    ...
    ('is_github_remote_url() {', "Branch-visibility helper centralizes GitHub-remote URL detection"),
    ('detect_github_remote() {', "Branch-visibility helper defines GitHub-remote autodetection"),
    ('github_remote_branch_parts() {', "Branch-visibility helper can parse explicit GitHub remote-tracking refs"),
    ('tracked_local_branch_for_ref() {', "Branch-visibility helper can map explicit GitHub remote-tracking refs back to tracked local branches"),
    ...
    ('printf \'%s\\t%s\\n\' "$remote" "$branch"', "Branch-visibility helper can emit parsed GitHub remote-tracking ref parts"),
    ("git for-each-ref --format='%(refname:short)\t%(upstream:short)' refs/heads |", "Branch-visibility helper scans local branches for matching tracked upstream refs"),
...
smoke_required = [
    ...
    ('"$HELPER" "refs/remotes/github/fix/test-visibility"', "Branch-visibility smoke test exercises the helper with an explicit GitHub remote-tracking ref"),
    ...
    ('Expected helper to resolve explicit GitHub remote-tracking refs back to the tracked local branch.', "Branch-visibility smoke test explains the explicit GitHub remote-tracking expectation"),
    ('Expected explicit GitHub remote-tracking ref to keep the tracked upstream visible.', "Branch-visibility smoke test keeps explicit GitHub remote-tracking upstream data visible"),
    ('Expected explicit GitHub remote-tracking ref to stay on the visible-branch path.', "Branch-visibility smoke test keeps explicit GitHub remote-tracking inputs on the branch-visibility path"),
