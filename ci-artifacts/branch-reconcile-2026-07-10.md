# Branch Reconcile Note — refreshed 2026-08-05 14:20 Asia/Taipei
...
- The latest non-handoff branch tip captured by this note is `4d4d32d`; later branch-reconcile-only note refreshes are intentionally ignored here so the handoff does not self-stale immediately on commit.
...
- Divergence count from `git rev-list --left-right --count fix/openclaw-config-path-and-local-mode...transplant/fix-openclaw-config-path-and-local-mode-clean-stack`:
  ...
  - `transplant/fix-openclaw-config-path-and-local-mode-clean-stack`: `189`
...
As of `4d4d32d`, the local branch-reconcile handoff is back in sync with the tree after the dirty-state guard, wrapper-meta-contract checkpoint settling, visible-handoff coverage, publish-parity handoff hardening, the later tip-handoff test pin, the clean-state settle commit, the newer publish-parity inferred-ref failure coverage, the refreshed VM verifier publish-boundary state commit, the follow-on clean-state settle commit, and the later wrapper-coverage checkpoint refresh/settle pair, and this note now treats later branch-reconcile-only refreshes as handoff-only bookkeeping rather than live branch-tip state.
