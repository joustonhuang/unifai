# Branch Reconcile Note — refreshed 2026-08-04 22:24 Asia/Taipei
...
- The latest non-handoff branch tip captured by this note is `28239e9`; later branch-reconcile-only note refreshes are intentionally ignored here so the handoff does not self-stale immediately on commit.
- The last non-doc tracked publish-boundary checkpoint remains `74d075e` until the current doc-only tip becomes GitHub-visible.
- Divergence count from `git rev-list --left-right --count fix/openclaw-config-path-and-local-mode...transplant/fix-openclaw-config-path-and-local-mode-clean-stack`:
  ...
  - `transplant/fix-openclaw-config-path-and-local-mode-clean-stack`: `185`
...
As of `28239e9`, the local branch-reconcile handoff is back in sync with the tree after the dirty-state guard, wrapper-meta-contract checkpoint settling, visible-handoff coverage, publish-parity handoff hardening, the later tip-handoff test pin, the clean-state settle commit, the newer publish-parity inferred-ref failure coverage, the refreshed VM verifier publish-boundary state commit, and the follow-on clean-state settle commit, and this note now treats later branch-reconcile-only refreshes as handoff-only bookkeeping rather than live branch-tip state.
