...
- the local hardening stack is currently ahead of that public ref through `04dff01` (`scripts: stop self-counting checkpoint refresh dirtiness`), 45 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `04dff01` (`scripts: stop self-counting checkpoint refresh dirtiness`) in:
