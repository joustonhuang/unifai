...
- the local hardening stack is currently ahead of that public ref through `da3d6a3` (`tests: pin publish history smoke coverage`), 36 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `da3d6a3` (`tests: pin publish history smoke coverage`) in:
