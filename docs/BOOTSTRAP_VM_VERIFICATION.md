...
- the local hardening stack is currently ahead of that public ref through `0160eb0` (`tests: harden vm host readiness contract coverage`), 61 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `0160eb0` (`tests: harden vm host readiness contract coverage`) in:
