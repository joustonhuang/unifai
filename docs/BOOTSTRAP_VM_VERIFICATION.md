...
- the local hardening stack is currently ahead of that public ref through `e291d8f` (`scripts: normalize refs-heads preflight inputs`), 21 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `e291d8f` (`scripts: normalize refs-heads preflight inputs`) in:
