...
- the local hardening stack is currently ahead of that public ref through `658ab2d` (`tests: lock verifier checkpoint tip-churn guards`), 8 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `658ab2d` (`tests: lock verifier checkpoint tip-churn guards`) in:
