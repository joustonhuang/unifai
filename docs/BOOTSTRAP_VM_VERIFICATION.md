...
- the local hardening stack is currently ahead of that public ref through `3bfb187` (`scripts: refresh verifier checkpoint drift shapes`), 80 commits ahead in total
- the local sandbox currently also carries 2 uncommitted publish-boundary maintenance path(s) beyond HEAD (`docs/BOOTSTRAP_VM_VERIFIER_CHECKPOINT_2026-06-15.md`, `scripts/refresh_vm_verifier_checkpoint_state.py`)
- the latest non-doc logic delta in that local stack is `3bfb187` (`scripts: refresh verifier checkpoint drift shapes`) in:
