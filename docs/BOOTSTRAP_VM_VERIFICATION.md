...
- the local hardening stack is currently ahead of that public ref through `eee71e3` (`scripts: fail preflight on dirty checkpoint refresh`), 43 commits ahead in total
...
- the latest non-doc logic delta in that local stack is `eee71e3` (`scripts: fail preflight on dirty checkpoint refresh`) in:
