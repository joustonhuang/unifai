...
- the local hardening stack is currently ahead of that public ref through `48f669b` (`scripts: narrow checkpoint delta path listing`), 53 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `48f669b` (`scripts: narrow checkpoint delta path listing`) in:
