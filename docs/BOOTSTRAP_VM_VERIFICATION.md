...
- the local hardening stack is currently ahead of that public ref through `35a7757` (`scripts: normalize refs-heads check-gate inputs`), 25 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `35a7757` (`scripts: normalize refs-heads check-gate inputs`) in:
