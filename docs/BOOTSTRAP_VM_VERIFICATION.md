...
- the local hardening stack is currently ahead of that public ref through `4e0dae3` (`scripts: derive verifier logic head from paths`), 73 commits ahead in total
- the local sandbox currently also carries 5 uncommitted publish-boundary maintenance path(s) beyond HEAD (`docs/BOOTSTRAP_VM_VERIFICATION.md`, `docs/BOOTSTRAP_VM_VERIFIER_CHECKPOINT_2026-06-15.md`, `scripts/check_vm_verifier_preflight_contract.py`, `scripts/run_vm_verifier_preflight.sh`, `scripts/smoke_test_vm_verifier_preflight_wrapper.sh`)
- the latest non-doc logic delta in that local stack is `4e0dae3` (`scripts: derive verifier logic head from paths`) in:
