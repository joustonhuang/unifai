...
- the local hardening stack is currently ahead of that public ref through `58bf2d5` (`tests: cover direct checkpoint blocker drift`), 64 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `58bf2d5` (`tests: cover direct checkpoint blocker drift`) in:
