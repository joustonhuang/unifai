...
- the local hardening stack is currently ahead of that public ref through `772ea55` (`tests: cover github branch visibility without remote`), 68 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `772ea55` (`tests: cover github branch visibility without remote`) in:
  ...
  - `scripts/smoke_test_github_branch_visibility_no_github_remote.sh`
