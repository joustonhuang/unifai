...
- the local hardening stack is currently ahead of that public ref through `74ca334` (`tests: cover forbidden head tip line`), 66 commits ahead in total
- the local sandbox currently carries no additional uncommitted publish-boundary maintenance delta
- the latest non-doc logic delta in that local stack is `74ca334` (`tests: cover forbidden head tip line`) in:
