...
- Latest tracked local head in the stack: `ef14bee`
- Latest non-doc logic head in the local stack: `ef14bee`
...
- The current local hardening stack has moved well beyond that earlier nine-commit checkpoint chain on top of the GitHub-visible branch: the latest tracked commit is now `ef14bee`, that same commit is also the latest non-doc logic head, the sandbox currently carries no additional uncommitted publish-boundary maintenance updates, and the branch is `ahead 82` over `fix/openclaw-config-path-and-local-mode`.
