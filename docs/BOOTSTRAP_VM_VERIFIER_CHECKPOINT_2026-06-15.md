...
- Latest tracked local head in the stack: `221a36f`
- Latest non-doc logic head in the local stack: `221a36f`
- Tracked local branch state at checkpoint: ahead by 35 commits over the GitHub-visible branch head
...
- The current local hardening stack has moved well beyond that earlier nine-commit checkpoint chain on top of the GitHub-visible branch: the latest tracked commit is now `221a36f`, that same commit is also the latest non-doc logic head, the sandbox currently carries 2 uncommitted publish-boundary maintenance updates, and the branch is `ahead 35` over `github/fix/openclaw-config-path-and-local-mode`.
