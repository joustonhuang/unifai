...
- Latest tracked local head in the stack: `2ee7536`
- Latest non-doc logic head in the local stack: `2ee7536`
- Tracked local branch state at checkpoint: ahead by 5 commits over the GitHub-visible branch head
...
- The current local hardening stack has moved well beyond that earlier nine-commit checkpoint chain on top of the GitHub-visible branch: the latest tracked commit is now `2ee7536`, that same commit is also the latest non-doc logic head, the sandbox currently carries no additional uncommitted publish-boundary maintenance updates, and the branch is `ahead 5` over `fix/openclaw-config-path-and-local-mode`.
