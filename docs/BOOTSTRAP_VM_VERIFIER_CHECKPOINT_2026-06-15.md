...
- Latest tracked local head in the stack: `e771a87`
- Latest non-doc logic head in the local stack: `e771a87`
- Tracked local branch state at checkpoint: ahead by 31 commits over the GitHub-visible branch head
...
- The current local hardening stack has moved well beyond that earlier nine-commit checkpoint chain on top of the GitHub-visible branch: the latest tracked commit is now `e771a87`, that same commit is also the latest non-doc logic head, the sandbox currently carries 2 uncommitted publish-boundary maintenance updates, and the branch is `ahead 31` over `github/fix/openclaw-config-path-and-local-mode`.
