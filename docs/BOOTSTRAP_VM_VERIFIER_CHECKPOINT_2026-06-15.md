...
- Latest tracked local head in the stack: `f497f07`
- Latest non-doc logic head in the local stack: `f497f07`
...
- The current local hardening stack has moved well beyond that earlier nine-commit checkpoint chain on top of the GitHub-visible branch: the latest tracked commit is now `f497f07`, that same commit is also the latest non-doc logic head, the sandbox currently carries 2 uncommitted publish-boundary maintenance updates, and the branch is `ahead 37` over `github/fix/openclaw-config-path-and-local-mode`.
