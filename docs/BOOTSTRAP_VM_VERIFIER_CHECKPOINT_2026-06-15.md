...
- The current local hardening stack has moved well beyond that earlier nine-commit checkpoint chain on top of the GitHub-visible branch: the latest tracked commit is now `a16bb5f`, that same commit is also the latest non-doc logic head, the sandbox currently carries no additional uncommitted checkpoint-refresh helper/doc updates, and the branch is `ahead 11` over `github/fix/openclaw-config-path-and-local-mode`.
